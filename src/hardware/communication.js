...
/** Получить все значения с ADAM */
export async function getHardwareValues(active_test) {
  ...
  // console.log("ADAM DATA: %o", ADAM_DATA);
...
function parseAnalog (analog, active_test) {
  ...
  let result = {
    test_press: {
      time:       elapsed,
      press_sys:  fromAnalog(CONFIG.adam.sys, analog, 4),
      press_top:  fromAnalog(CONFIG.adam.top, analog, 4),
      press_btm:  fromAnalog(CONFIG.adam.btm, analog, 4),
    },
    test_power: {
      time:       elapsed,
      rpm:        fromAnalog(CONFIG.adam.rpm, analog, 4),
      torque:     fromAnalog(CONFIG.adam.trq, analog, 4),
      temper:     fromAnalog(CONFIG.adam.tmp, analog, 4),
    },
  }
